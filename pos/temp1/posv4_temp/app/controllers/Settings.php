...
class Settings extends MY_Controller
{
    ...
    function index() {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                'site_name' => DEMO ? 'Innowave' : $this->input->post('site_name'),
                'language' => $this->input->post('language'),
                'tel' => $this->input->post('tel'),
                'currency_prefix' => DEMO ? 'USD' : strtoupper($this->input->post('currency_prefix')),
                'default_tax_rate' => $this->input->post('tax_rate'),
                'service_charges' => $this->input->post('sc'),// mod by mtk at 26 Feb 2019 for service charges 
...
$this->session->set_flashdata('message', lang('setting_updated'));
	...
	// #mod File backup location update by Nyi Nyi at 7/4/2017 
        //$this->data['files'] = glob('./files/backups/*.zip', GLOB_BRACE);
	//$this->data['files'] = glob('../backups/*.zip', GLOB_BRACE);
	// #mod File backup location update by Nyi Nyi at 21/2/2018 
	$this->data['files'] = glob(config_item('backup_location') . '*.zip', GLOB_BRACE);
	
	
	// #mod File backup location update by Nyi Nyi at 7/4/2017 
        //$this->data['dbs'] = glob('./files/backups/*.txt', GLOB_BRACE);
	//$this->data['dbs'] = glob('../backups/*.txt', GLOB_BRACE);
	// #mod File backup location update by Nyi Nyi at 21/2/2018 
	$this->data['dbs'] = glob(config_item('backup_location') . '*.txt', GLOB_BRACE);
	...
	// #mod File backup location update by Nyi Nyi at 7/4/2017 | 21/2/2018
        //$save = './files/backups/' . $db_name;
		$save = config_item('backup_location') . $db_name;
	...
	$this->tec->zip("./", config_item('backup_location'), $name);
	...
	$file = file_get_contents(config_item('backup_location') . $dbfile . '.txt');
	...
	$this->zip->read_file(config_item('backup_location') . $dbfile . '.txt');
	...
	force_download(config_item('backup_location') . $zipfile . '.zip', NULL);
        exit();
    }
	...
	$file = config_item('backup_location') . $zipfile . '.zip';
        $this->tec->unzip($file, './');
	...
	unlink(config_item('backup_location') . $dbfile . '.txt');
	...
	unlink(config_item('backup_location') . $zipfile . '.zip');
