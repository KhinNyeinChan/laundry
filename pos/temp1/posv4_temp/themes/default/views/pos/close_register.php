...
<div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
			...
			<?php 
			echo '<div align="center"><img src="'.base_url('uploads/'.$store->logo).'" alt="'.$store->name.'"></div>';
			...
			<!-- #mod #mod1 close register timestamp created by zay yar at 1/3/2017 -->
			<?php 
			$now = new DateTime();
			echo "<h4 class='modal-title'>". "(Closed at: " . $now->format('D d M Y g:i A') . ")</h4>";
				?>
        </div>

        <?= form_open("pos/close_register/" . $user_id); ?>
        <div class="modal-body">
            <?php if (($this->Staff) && config_item('staff_view_sales') || ($this->Admin) || ($this->Manager)) { ?>     
  ...
  <?php } ?>
	...
	<div class="no-print">
			...
			/* Mod Nyi Nyi */
			echo '<hr><h4>' . lang('opened_bills') . '</h4><table class="table table-hovered table-bordered"><thead><tr><th>' . lang('customer') . '</th><th>' . lang('date') . '</th><th>' . lang('reference') . '</th><th>' . lang('amount') . '</th>';
			...
			echo '</tr></thead><tbody>';
				...
				echo '<tr><td>' . $bill->customer_name . '</td><td>' . $this->tec->hrld($bill->date) . '</td><td class="col-xs-4">' . $bill->hold_ref . '</td><td class="text-right">' . $bill->grand_total . '</td>';
				if($this->Admin) { 
					echo '<td class="text-center"><a class="tip" title="' . lang("delete_bill") . '" href="' . site_url('sales/delete_holded/' . $bill->id) . '" onclick="return confirm(\''.lang('alert_x_holded').'\')"><i class="fa fa-trash-o"></i></a></td>';
				}
				echo '</tr>';
                    }
                    echo '</tbody></table>';
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default pull-left no-print" data-dismiss="modal"><?=lang('close')?></button>
                <?php
                if ( ! $Settings->remote_printing) {
                    ...
                    echo '<button id="print-register-details" class="btn btn-default no-print">'.lang("print").'</button>';
                } else {
                    echo '<button type="button" onclick="window.print();" class="btn btn-default no-print">'.lang('print').'</button>';
                }
                ?>
                <?= form_submit('close_register', lang('close_register'), 'class="btn btn-primary no-print"'); ?>
            </div>
			</div> <!--end no-print -->
