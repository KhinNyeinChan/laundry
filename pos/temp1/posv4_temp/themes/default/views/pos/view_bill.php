...
<div class="wrap">
    <div class="bill" id="bill">
		<!-- mod by Nyi Nyi to display vertical scroll bar on 4th July 2018 -->
		<!-- <div id="product-list"> -->
        <div id="product-list" style="overflow-y: auto">
		<!-- end mod -->
        ...
        <div id="totals">
            <table style="width:100%; float:right; padding:5px; color:#000; background: #FFF;" id="totalTable">
                ...
                <tr>
                    <td style="border-left: 1px solid #ddd; padding-left:10px; text-align:left; "><?= lang('rounding'); ?></td>
                    <td style="text-align:right; padding-right:10px; font-weight:bold;">
                     <span id="rounding">0.00</span></td></tr>
                     <!-- mod by mtk at 26 Feb 2019 for service charges -->
                    <td style="padding-left:10px; text-align:left; font-weight:normal;"><?= lang('service_charges'); ?></td>
                    <td style="border-right: 1px solid #ddd; text-align:right; padding-right:10px; font-weight:bold;"><span id="sc_con">0.00</span></td>
                    <!-- end mod by mtk --> 
                <tr>
                    <td colspan="2"
...
function roundNumberr2($gtotal,$div){
    ...
    if (get('spositems')) {
        ...
        if (sc.indexOf("%") !== -1) {
            var pts = sc.split("%");
            service_charges = ((total-order_discount)*parseFloat(pts[0]))/100;
        } else {
        ...
        var g_total = total - parseFloat(order_discount) + parseFloat(order_tax) + parseFloat(service_charges);// mod by mtk at 26 Feb 2019 for service charges 
        //alert(g_total);

        if(Settings.rounding==1){
        round_total=roundNumberr(g_total,50);

        }
         if(Settings.rounding==2){
        round_total=roundNumberr1(g_total,50);

        }
        if(Settings.rounding==3){
         round_total=roundNumberr2(g_total,50);
        }
        var rounding=round_total - g_total;
       // alert(rounding);
        grand_total = formatMoney(g_total);
         $("#total-payable").text(round_total);
         $("#total").text(formatMoney(total));
         $("#rounding").text(formatMoney(rounding));
