...
class Reports extends MY_Controller
{
    ...
    function daily_sales($year = NULL, $month = NULL) {
        ...
        if (!empty($sales)) {
            foreach ($sales as $sale) {
                ...
                if (config_item('is_restaurant') && config_item('service_charges')){ 
                ...
                "</td><td style='text-align:right;'><span style='font-weight:normal;'>{$this->tec->formatMoney($sale->product_tax)}<br>{$this->tec->formatMoney($sale->order_tax)}</span><br>{$this->tec->formatMoney($sale->total_tax)}</td></tr><tr><td class='violet'>".lang('service_charges'). // mod by mtk at 26 Feb 2019 for service charges
                "</td><td style='text-align:right;'>{$this->tec->formatMoney($sale->service_charges)}</td></tr><tr><td class='violet'>".lang('discount').// mod by mtk at 26 Feb 2019 for service charges
    ...
    function get_sales() {
        ...
        $this->datatables
        ->select("id, date, customer_name, total, total_tax, total_discount, grand_total, paid, (grand_total-paid) as balance, status, service_charges")// mod by mtk at 26 Feb 2019 for service charges
        ->from('sales');
        if ($this->session->userdata('store_id')) {
            $this->datatables->where('store_id', $this->session->userdata('store_id'));
        }
	
	//mod nyi nyi 13th July 2017
        //$this->datatables->unset_column('id');
	//end mod 
        if($customer) { $this->datatables->where('customer_id', $customer); }
        if($user) { $this->datatables->where('', $user); }
    ...
    function sale_details()
    {
        ...
        $this->data['saledetails'] = $this->reports_model->getSaleDetails();
        $this->data['page_title'] = $this->lang->line("sale_details_report");
        $this->data['page_title'] = $this->lang->line("sale_details_report");
        $bc = array(array('link' => '#', 'page' => lang('reports')), array('link' => '#', 'page' => lang('sale_details_report')));
        $meta = array('page_title' => lang('sale_details_report'), 'bc' => $bc);
        $this->page_construct('reports/sale_details', $this->data, $meta);
    }
    ...
    function get_sale_details() {
        ...
        $this->load->library('datatables');
        $this->datatables
        //->select("id, sale_id, quantity, unit_price, subtotal, product_code, product_name")
         ->select($this->db->dbprefix('sale_items').".id as id, ".$this->db->dbprefix('sale_items').".sale_id, ".$this->db->dbprefix('sale_items').".quantity, ".$this->db->dbprefix('sale_items').".unit_price, ".$this->db->dbprefix('sale_items').".subtotal, ".$this->db->dbprefix('sale_items').".product_code, ".$this->db->dbprefix('sale_items').".product_name, ".$this->db->dbprefix('sale_items').".real_unit_price, ".$this->db->dbprefix('sales').".date, ".$this->db->dbprefix('sales').".customer_name, ".$this->db->dbprefix('sales').".status, ".$this->db->dbprefix('sale_items').".foc as foc, ", FALSE)
        ->from('sale_items')
