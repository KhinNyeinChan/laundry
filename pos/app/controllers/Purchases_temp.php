...
function expense_category_list(){
    if (($this->Admin) || ($this->Manager)) {
        ...
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['page_title'] = lang('expense_category_list');
        $bc = array(array('link' => '#', 'page' => lang('expense_category')));
        $meta = array('page_title' => lang('expense_category'), 'bc' => $bc);
        $this->page_construct('purchases/expense_category_list', $this->data, $meta);
}
...
function get_expense_category(){
     if (($this->Admin) || ($this->Manager)) {
        ...
        $this->datatables->from('expenses_category');
        //$this->datatables->where('store_id', $this->session->userdata('store_id'));
        $this->datatables->add_column("Actions", "<div class='text-center'><div class='btn-group'><a href='" . site_url('purchases/edit_expense_category/$1') . "' title='" . lang("edit_purchase") . "' class='tip btn btn-warning btn-xs'><i class='fa fa-edit'></i></a> <a href='" . site_url('purchases/delete_expense_category/$1') . "' onClick=\"return confirm('" . lang('alert_x_expense_category') . "')\" title='" . lang("delete_expense_category") . "' class='tip btn btn-danger btn-xs'><i class='fa fa-trash-o'></i></a></div></div>", "id");
        ...
        if ($this->purchases_model->deleteExpenseCategory($id)) {
            $this->session->set_flashdata('message', lang("expense_category_deleted"));
            redirect('purchases/expense_category_list');
        }
    }
...
function edit_expense_category($id = NULL) {
     ...
     $this->form_validation->set_rules('expense_category', lang("expense_category"), 'required');
       
         if ($this->form_validation->run() == true) {
            $data = array( 
                'name' => $this->input->post('expense_category')
            );

        }
         elseif ($this->input->post('expense_category')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect($_SERVER["HTTP_REFERER"]);
        }

        if ($this->form_validation->run() == true && $this->purchases_model->updateExpenseCategory($id, $data)) {
            $this->session->set_flashdata('message', lang("expense_category_updated"));
            redirect("purchases/expense_category_list");
        } else {

            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['expense'] = $this->purchases_model->getExpenseCategoryByID($id);
            $this->data['expense_category'] = $this->purchases_model->getExpenseCategoryByID($id);
            $this->data['page_title'] = lang('edit_expense_category');
            $bc = array(array('link' => site_url('purchases'), 'page' => lang('purchases')), array('link' => site_url('purchases/expenses'), 'page' => lang('expenses')), array('link' => '#', 'page' => lang('edit_expense_category')));
            $meta = array('page_title' => lang('edit_expense_category'), 'bc' => $bc);
            $this->page_construct('purchases/edit_expense_category', $this->data, $meta);
   ...
   function get_purchasess() {
        ...
        $this->load->library('datatables');
        $this->datatables->select($this->db->dbprefix('purchases') . ".id as id, date, reference, total, note, attachment, ".$this->db->dbprefix('purchase_items').".quantity");
        $this->datatables->from('purchases')
...
$this->form_validation->set_rules('expense_category', lang("expense_category"), 'required');
 ...
 if ($this->form_validation->run() == true && $this->purchases_model->addExpenseCategory($data)) {
            ...
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['page_title'] = lang('add_expense_category');
            $bc = array(array('link' => site_url('purchases'), 'page' => lang('products')), array('link' => '#', 'page' => lang('add_expense_category')));
            $meta = array('page_title' => lang('add_expense_category'), 'bc' => $bc);
            $this->page_construct('purchases/expense_category_add', $this->data, $meta);
    ...
    function add() {
        ...
        //#MOD #MOD1 Manager can Add by Zay Yar at 10/10/2017
        //Manager can add 
        if (($this->Admin) || ($this->Manager)) {
    }else {
            redirect('pos');
    ...
    function get_expenses($user_id = NULL) {
        ...
        if ($this->db->dbdriver == 'sqlite3') {
            $this->datatables->select($this->db->dbprefix('expenses') . ".id as id, date, reference, amount, note, ".$this->db->dbprefix('expenses_category').".name, (" . $this->db->dbprefix('users') . ".first_name || ' ' || " . $this->db->dbprefix('users') . ".last_name) as user, attachment", FALSE);
        } else {
            $this->datatables->select($this->db->dbprefix('expenses') . ".id as id, date, reference, amount, note, ".$this->db->dbprefix('expenses_category').".name, CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ', " . $this->db->dbprefix('users') . ".last_name) as user, attachment", FALSE);
        }
        $this->datatables->from('expenses')
            ->join('users', 'users.id=expenses.created_by', 'left')
            ->join('expenses_category','expenses.ecategory_id=expenses_category.id','left')
	...
	*/
    ...
    function add_expense() {
        $this->data['expenses_categories']=$this->purchases_model->getExpenseCategory();
        if ( ! $this->session->userdata('store_id')) {
            $this->session->set_flashdata('warning', lang("please_select_store"));
            redirect('stores');
        }
        ...
        $this->form_validation->set_rules('expense', lang("expense"), 'required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'created_by' => $this->session->userdata('user_id'),
                'store_id' => $this->session->userdata('store_id'),
                'ecategory_id' =>$this->input->post('expense'),
                'note' => $this->input->post('note', TRUE)
            );
    ...
    function edit_expense($id = NULL) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'amount' => $this->input->post('amount'),
                'note' => $this->input->post('note', TRUE),
                'ecategory_id' => $this->input->post('expense_id'),
            );
            if ($_FILES['userfile']['size'] > 0) {
                $this->load->library('upload');
                $config['upload_path'] = 'uploads/';
