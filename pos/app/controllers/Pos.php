...
class Pos extends MY_Controller {
    ...
    function index($sid = NULL, $eid = NULL) {
        ...
        if( $this->input->post('did') ) { $did = $this->input->post('did'); } 
        else { $did = NULL; }
        if($eid && !$this->Admin) {
        ...
        //mod by Nyi Nyi on 23rd Jan 2018 to void open register for waiter
        if ($this->Waiter || $this->Mobile_Caisher) {
            //redirect(isset($_SERVER["HTTP_REFERER"]) ? $_SERVER["HTTP_REFERER"] : 'pos');
             $register_data = array('register_id' => $register->id, 'cash_in_hand' => '0', 'register_open_time' => $register->date);
            $this->session->set_userdata($register_data);
        } else {
            
        //mod by Nyi Nyi on 23rd Jan 2018 to replace waring message instead of error message
       ...
       if($print_order==3){
          $ono=$this->pos_model->MaxOrderNumber(); 
       }
     
       if(!$suspend && $print_order!=3)
       {
         $ono=0;
       }
       
      if(config_item('capacity') && config_item('is_restaurant'))
      {
        if($_POST['capacity'])
        {
        ...
        $cancel_no=$_POST['cancel_order_no'];
        $produ_id=$_POST['item_id'];
     ...
     $this->form_validation->set_rules('customer', lang("customer"), 'trim|required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $product_tax = 0;
            $order_tax = 0;
        //mod nyi nyi on 9th Jan 2018 for service charges
            $service_charges = 0;
        //end mod 
            $product_discount = 0;
            $order_discount = 0;
        ...
        if($_POST['onoo'][$r]>0){
               $ono = $_POST['onoo'][$r];
                ...
                $item_foc = $_POST['item_foc'][$r];// mod by mtk for showing foc at 21 March 2019
                $item_ordered = $_POST['item_was_ordered'][$r];
                $additional_order = $_POST['additional_order'][$r];
   
                $item_discount = isset($_POST['product_discount'][$r]) ? $_POST['product_discount'][$r] : '0';
 
           $suspended_sale = $this->pos_model->getAllSuspendedCustomer();
           ...
           if (isset($item_id) && isset($real_unit_price) && isset($item_quantity)) {
                    ...
                    if (isset($item_discount)) {
                        ...
                        if ($dpos !== false) {
                            ...
                            $pr_discount = (($this->tec->formatDecimal($whole_sale)) * (Float)($pds[0])) / 100; //mod by mtk
                            } else
                              {*/
                    ...
                    $products[] = array(
			            ...
			            'foc' => $item_foc,  //mod by mtk for showing foc at 21 March 2019
        ...
        //mod nyi nyi on 9th Jan 2018 for service charges
        /*
         if (config_item('is_restaurant') && config_item('service_charges')>0){
            $service_charges = $this->tec->formatDecimal($total / config_item('service_charges')) ;
                }
        */
        //end mod 
        ...
        //end mod 
           // mod by mtk at 26 Feb 2019 for service charges   
            ...
            if($this->input->post('service_charges')) {
                $sc_id = $this->input->post('service_charges');
                
                $opos = strpos($sc_id, $percentage);
                if ($opos !== false) {
                    $ots = explode("%", $sc_id);
                    $service_charges = $this->tec->formatDecimal((($total +$product_tax- $order_discount) * (Float)($ots[0])) / 100);
                } else {
                    $service_charges = $this->tec->formatDecimal($sc_id);
                }
            ...
            $grand_total = $this->tec->formatDecimal($this->tec->formatDecimal($total) + $total_tax - $order_discount + $service_charges);// mod by mtk at 26 Feb 2019 for service charges 
            ...
            $w_sale = $_POST['whs'];
            $this->data['wh_sale']=$this->pos_model->Seperator($w_sale);
           
            //if ($customer_details->id == 1 && $paid < $round_total) {
    //mod by Nyi Nyi on 22nd Feb 2018
        
    if (($customer_details->id == 1 && $paid < $round_total)
|| (substr($customer_details->name,0,5) == "Table" && $paid < $round_total  && (!$suspend))
|| (substr($customer_details->name,0,8) == "Takeaway" && $paid < $round_total && (!$suspend)))  {
           ...
           foreach ($suspended_sale as $aa) {
               
              if($customer_details->name==$aa->customer_name){
                  $this->session->set_flashdata('error', lang('table_is_already_hold '));
                    redirect($_SERVER["HTTP_REFERER"]);
               }
            }
              }
    ...
    //}
       ...
       // $del_id = $this->input->post('item_id');
            if(($_POST['cancel_order_no']) && ($_POST['item_id'])){
...
if($additional_order){
    $this->pos_model->updateOrder($data1,$product2,$ono);
    $this->pos_model->updateOrderStatus($cancel_no,$produ_id,$date,$reason,$res_person);
     }
 }
  if(!config_item('order_tracking')){
    $ono=0;
  }
        ...
        { 
           ...
           //$order_count=5;
            if($suspend) {
                unset($data['status'], $data['rounding'], $data['service_charges']);
                ...
                if($sale = $this->pos_model->addSale($sale_data, $products, $payment, $did)) {
                    $this->pos_model->updateSaledOrder($ono);
                    $this->session->set_userdata('rmspos', 1);
                    $msg = lang("sale_added");
        ...
        else
        {
            ...
            if(isset($sid) && !empty($sid)) {
                ...
                foreach ($inv_items as $item) {
                    ...
                    $row->foc = $item->foc;//mod by mtk for showing foc at 21 March 2019
                    $row->ordered = $item->quantity;
                    $row->ordered_foc = $item->foc;
		            $row->order_no =$item->order_id;
                    //redirect("pos/".$did);
                    $combo_items = FALSE;
            ...
            $this->data['reference_note'] = isset($sid) && !empty($sid) ? $suspended_sale->hold_ref : (isset($eid) && !empty($eid) ? $sale->hold_ref : NULL);
              ...
              $this->data['suspend_order_id'] = isset($sid) && !empty($sid) ? $suspended_sale->order_id : (isset($eid) && !empty($eid) ? $sale->hold_ref : NULL);
             
            $this->data['capacity']=$suspended_sale->capacity;
            
     
            $this->data['order_idd']=$this->pos_model->MaxOrderNumber();
    ...
    function get_product($code = NULL) {
        ...
        if($product = $this->pos_model->getProductByCode($code)) {
            ...
            $product->qty = 1;
            $product->comment = '';
            $product->foc = '0'; //mod by mtk for showing foc at 21 March 2019
            $product->discount = '0';
            $product->price = $product->store_price > 0 ? $product->store_price : $product->price;
            $product->real_unit_price = $product->price;
            //$product->real_whole_sale = $product->details;//mod by mtk
...
//                unset($row->cost, $row->details);
                $row->qty = 1;
                $row->comment = '';
                $row->foc = '0'; //mod by mtk for showing foc at 21 March 2019
                $row->discount = '0';
                $row->price = $row->store_price > 0 ? $row->store_price : $row->price;
                $row->real_unit_price = $row->price;
                //$product->real_whole_sale = $product->details;
    ...
    $date = date('Y-m-d H:i:s');
        ...
        for ($r = 0; $r < $i; $r++) {
            ...
            $item_foc = $_POST['item_foc'][$r]; //mod by mtk for showing foc at 21 March 2019
            $item_ordered = $_POST['item_was_ordered'][$r];
            $item_foc_ordered = $_POST['item_foc_ordered'][$r];
            $item_discount = isset($_POST['product_discount'][$r]) ? $_POST['product_discount'][$r] : '0';
            ...
            if (isset($item_id) && isset($real_unit_price) && isset($item_quantity)) {
                ...
                if (isset($product_details->tax) && $product_details->tax != 0) {
                    ...
                    if ($product_details && $product_details->tax_method == 1) {
                        ...
                        $tax = $product_details->tax . "%";
                            ...
                            $item_tax = $this->tec->formatDecimal((($whole_sale) * $product_details->tax) / (100 + $product_details->tax));
                        $tax = $product_details->tax . "%";
                        $item_net_price -= $item_tax;
                    }
                ...
                $product_tax += $pr_item_tax;
                $subtotal = (($item_net_price * $item_quantity) + $pr_item_tax);
                $printer_id = $this->site->getPrinter($category_id);
                $p_id = $printer_id->printer_id;
                $product1[]=array(
                      'product_id' => $item_id,
                      'product_code' => $product_code,
                      'product_name' => $product_name,
                ...
                $products[] = (object) array(
                    ...
                    'discount' => $item_discount,
                    'comment' => $item_comment,
                    'foc' => $foc, //mod by mtk for showing foc at 21 March 2019
                    'item_discount' => $pr_item_discount,
                    'tax' => $tax,
                    'item_tax' => $pr_item_tax,
                    'subtotal' => $subtotal,
    ...
    $item->qty = $item->quantity - ($item->ordered ? $item->ordered : 0);
    $item->foc = $item->foc - ($item->foc_ordered ? $item->foc_ordered : 0);
    ...
    if ($item->qty >0 || $item->foc >0)
    {
        $pid=$item->product_id;
        $pcode=$item->product_code;
        $pname=$item->product_name;
        $quantity=$item->qty + $item->foc;
        ...
        $t_qty = $this->input->post('total_quantity');
        $total_qty = $t_qty + $item->foc;
        $total_tax = $this->tec->formatDecimal($product_tax + $order_tax);
      ...
      //$this->data['order_idd']=$this->pos_model->MaxOrderNumber();
       ...
       if(config_item('order_tracking')){ 
        ...
        $this->data['cname']=$o_count;



    

if(!$additional_order && $o_count == 0){

 $order_id=$this->pos_model->insertOrder($data1,$product2);
 //$this->pos_model->updateSaledOrder($ono);
}
else{
    $this->pos_model->updateOrder($data1,$product2,$ono);
  ...
  if($this->Settings->remote_printing==0){
        if ($bo == 'bill') {
            $printer = $this->site->getPrinterByID($this->Settings->printer);
            $this->load->library('escpos');
            $this->escpos->load($printer);
            ...
            $cancel_number=$this->input->post('cancel_order_no');
            $o_number=$this->input->post('delete_o_id');
            $res=$this->input->post('reason');
            $res_personn=$this->input->post('res_person');
            $pp=array();

            if($this->input->post('del_name')){
             $pp[] = (object) array(
                    'product_code' => $del_code,
                    'product_name' => $del_name,
                    'quantity' => 1,
                    'status' => 'Cancelled',
                    
                );
             $all_row = $this->pos_model->getPrinterForCancel($del_id);
            $printer_no = $all_row->printer_id;
            }
            if(empty($pp)){
            $this->form_validation->set_rules('product', lang("order_items"), 'required');
            }
            else{   
                $this->pos_model->updateOrderStatus($cancel_number,$del_id,$date,$res,$res_personn);
                 $this->load->library('escpos');
                 $printeer = $this->site->getPrinterByID($printer_no);
                    $this->escpos->load($printeer);
                    $this->escpos->print_orderr($cancel_number,$store, $data, $pp, $created_by);
            }
        }
                foreach ($order_printers as $printer_id) {
                  $p=array();
                  foreach ($products as $product) {
                    ...
                    if($product->printer_id == $printer_id){

                     $pr_id = $product->product_id;
                     $pr_cid = $product->category_id;
                     $pr_pid = $product->printer_id;
                     $pr_qty = $product->quantity;
                     $pr_uprice = $product->unit_price;
                     $pr_nuprice = $product->net_unit_price;
                     $pr_ds = $product->discount;
                     $pr_cm = $product->comment;
                     $pr_ids = $product->item_discount;
                     $pr_tax = $product->tax;
                     $pr_itax = $product->item_tax;
                     $pr_stotal = $product->subtotal;
                     $pr_ruprice = $product->real_unit_price;
                     $pr_cost = $product->cost;
                     $pr_code = $product->product_code;
                     $pr_pname = $product->product_name;
                     $pr_ordered = $product->ordered;
                     $pr_foc = $product->foc; //mod by mtk for showing foc at 21 March 2019
                     $pr_foc_ordered = $product->foc_ordered;
                    ...
                    'printer_id' => $pr_pid,
                    'quantity' => $pr_qty,
                    'unit_price' => $pr_uprice,
                    'net_unit_price' => $pr_nuprice,
                    'discount' => $pr_ds,
                    'comment' => $pr_cm,
                    'item_discount' => $pr_ids,
                    'tax' => $pr_tax,
                    'item_tax' => $pr_itax,
                    'subtotal' => $pr_stotal,
                    'real_unit_price' => $pr_ruprice,
                    'cost' => $pr_cost,
                    'product_code' => $pr_code,
                    'product_name' => $pr_pname,
                    'ordered' => $pr_ordered,
                    'foc' => $pr_foc,
                    'foc_ordered' => $pr_foc_ordered,  // mod by mtk for showing foc at 21 March 2019
                    );
                ...
                if (empty($p)) {
                   $this->form_validation->set_rules('product', lang("order_items"), 'required');
                    ...
                    if($additional_order && $o_count == 1)
                    {
                    $this->load->library('escpos');
                    $printer = $this->site->getPrinterByID($printer_id);
                    $this->escpos->load($printer);
                    $this->escpos->print_order($o_number,$store, $data, $p, $created_by);
                    }
                    if(!$additional_order && $o_count == 0)
                    {
                    $this->load->library('escpos');
