...
class Escpos
{
    ...
    function print_data($data) {
        ...
        if (isset($data->items) && !empty($data->items)) {
            ...
            foreach ($data->items as $item) {
                ...
                $this->printer->text($this->product_name(addslashes($item->product_name)). "\n");
                //end mod 
    ...
    function print_order($no,$store, $sale, $items, $created_by) {

    // mod by amw and mtk for multiple order printer at 20 Feb 2019 *** changes can view compare with version 2.2.9
    ...
    $item->foc = $item->foc - ($item->foc_ordered ? $item->foc_ordered : 0);
		...
		//$this->printer->text($this->printLine($this->product_name(addslashes($item->product_name)).' ('.$item->product_code.') : [ ' .
                 //       ($item->quantity > 0 ? $this->tec->formatQuantity($item->quantity) : 'xxxx')." ]"). "\n");
	...
	if ($item->quantity >0 || $item->foc >0){

        $item->quantity = $item->quantity + $item->foc; //mod by mtk for showing foc at 21 March 2019
    ...
    */
    ...
    //end mod 
     ...
     $this->printer->text('Ref: ' . $sale->hold_ref . "\n");
   /* $this->printer->text('Order Number: ' . $sale->order_id . " | ". " Ref".$sale->hold_ref . "\n");
*/
     $this->printer->text('Order Number:' .$no. "\n");
     $this->printer->text('Sales Person: ' . $created_by->first_name." ".$created_by->last_name . "\n");
     $this->printer->text('Date: '.$this->tec->hrld($sale->date) . "\n");
     $this->printer->feed();
     ...
     $this->printer->text($this->printLine($this->product_name(addslashes($item->product_name)).' ('.$item->product_code.') : [ ' .
     $this->tec->formatQuantity($item->quantity)." ]"). "\n");
	...
	if ($additional_order) {
		$this->printer->text('***Additional Order***'."\n");
        //$this->printer->text('***Testing Order***'."\n");
	}
...
function print_orderr($cancel,$store, $sale, $items, $created_by) {
    // mod by amw and mtk for multiple order printer at 20 Feb 2019 *** changes can view compare with version 2.2.9
   ...
   /* $this->printer->text('Order Number: ' . $sale->order_id . " | ". " Ref".$sale->hold_ref . "\n");
*/
     $this->printer->text('Order Number:' .$cancel. "\n");
     $this->printer->text('Sales Person: ' . $created_by->first_name." ".$created_by->last_name . "\n");
     $this->printer->text('Date: '.$this->tec->hrld($sale->date) . "\n");
     $this->printer->feed();
