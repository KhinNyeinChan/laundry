...
class Site extends CI_Model
{
    ...
    public function getProductByID($id, $store_id = NULL) {
        ...
        //$this->db->select("{$this->db->dbprefix('categories')}.*")
	...
	//mod by Nyi Nyi on 23rd Jan 2018 to allow admin/manager/staff & waiter to view all opened bills so that more items can be added 
	//if (($this->Waiter) || ($this->Admin) || ($this->Manager)){
    ...
    // mod by amw and mtk for multiple order printer at 20 Feb 2019
    public function getPrinter($id){
        $q = $this->db->get_where('categories', array('id' => $id), 1);
        //$q = $this->db->select('printer_id')
        //->from('categories')
        //->where('id' => $id);
        if ( $q->num_rows() > 0 ) {
            return $q->row();
