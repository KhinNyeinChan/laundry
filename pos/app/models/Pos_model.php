...
public function updateSaledOrder($sor_id)
{
    
    $this->db->update('order_items', array('status' => 'Completed'), array('order_id' => $sor_id ,'status' => 'Ordered'));

}

public function getunit($id){
    $this->db->select('products.unit_id,unit.name');
$this->db->from('products');
$this->db->join('unit', 'unit.id = products.unit_id','left');
$this->db->where('products.id',$id);
 //$this->db->select('unit.name as unit_name')
          //  ->join('unit', 'unit.id=products.unit_id', 'left')
          //  ->group_by('products.id');
            //->order_by('id', 'asc');
        //$q = $this->db->get_where('products', array('id' => 10));
    $q = $this->db->get();
  if ($q->num_rows() > 0) {
            return $q->row();
        }
...
public function getBrand($id){
    $this->db->select('products.brand_id,brands.name');
    $this->db->from('products');
...
public function updateOrderStatus($order_no,$pro_id,$date,$reason,$res_person){
$this->db->set('status','Cancelled'); 
 $this->db->set('reason',$reason);
  $this->db->set('modified_time',$date);
 $this->db->set('responsible_person',$res_person);  
...
public function update_Status($order_no){
 $this->db->set('status','Cancelled');   
 $this->db->where('order_id',$order_no);
 $this->db->update('order_items');
 return True;
...
public function MaxOrderNumber(){
    ...
    public function getPrinterForCancel($product_id){
    $this->db->select('*');
    $this->db->from('products'); 
    ...
    //mod Remove Raw product items from POS screen by Nyi Nyi at 7/4/2017 -->
    //$this->db->where('category_id', $category_id);
    $where = "category_id=" . $category_id . " AND type <> 'Raw' ";
    $this->db->where($where);
    ...
    public function getMaxOrders(){
        ...
        $q=$this->db->query("SELECT MAX(order_no) FROM `orders`");
       // return $q;
        if( $q->num_rows() > 0 )
          {
            foreach ($q->result() as $row) {
     ...
     public function getAllSuspendedCustomer(){
    ...
    public function getAllSuspendedCustomers($id){
         $q = $this->db->get_where('suspended_items', array('id' => $id), 1);
          if( $q->num_rows() > 0 ) {
            return $q->row();
          }
          return FALSE;
    ...
    public function updateSale($id, $data, $items) {
        ...
        $data['status'] = $data['grand_total'] > $osale->paid && $osale->paid > 0 ? 'partial' : ($data['grand_total'] <= $osale->paid ? 'paid' : 'due');
    ...
    public function updateOrder($data,$items,$oid){
    unset($data['start_date']);
         if($this->db->update('orders', $data, array('id' => $oid))) {
                foreach ($items as $item) {
                    unset($item['cost']);
                    $item['order_id'] = $oid;
                    $this->db->insert('order_items', $item);
   ...
   public function getSaleByID($sale_id) {
    ...
    public function getStartTime($or_id){
         $q = $this->db->get_where('orders', array('id' => $or_id), 1);
          if( $q->num_rows() > 0 ) {
            return $q->row();
          }
          return FALSE;
    }
  public function getTime($ono){
       $q = $this->db->get_where('orders', array('id' => $ono), 1);
          if( $q->num_rows() > 0 ) {
            return $q->row();
          }
          return FALSE;
  ...
  $this->db->join('suspended_sales', 'suspended_items.suspend_id = suspended_sales.id');
  $q = $this->db->get();
         if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
